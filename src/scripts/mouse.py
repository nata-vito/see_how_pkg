...
class Mouse:
    ...
    # Callback to save data from command ROS message field
    def mouseHandCallback(self, data):
        self.command = data.command
       
        
    # Subscriber to Right hand 
    def mouseHandListener(self):
        rospy.Subscriber('Right', Hand, self.mouseHandCallback)


    # Get landmarks from the hand
    def findHandLm(self):
    ...
    # Checking the gesture for mouse control
    def isCursor(self):      
        return True if (self.fingers == [1, 1, 1, 1, 1]) and (self.active == 0) else False
    
    
    # Checking the scroll control gesture
    def isScroll(self):
        return True if (self.fingers == [0, 1, 0, 0, 0] or self.fingers == [0, 1, 1, 0, 0]) and (self.active == 0) else False
        
        
    # Checking the gesture to disable scroll/mouse
    def isActivate(self):
        return True if self.fingers == [0, 0, 0, 0, 0] else False
        
        
    # Decision to activate the mouse functions 
    def decisionTaking(self):
        
        self.mouseHandListener() # Checking the command obtained from the ROS topic
    ...
    # Processing and generating the results                        
    def main(self, img = None, flag = False):
        ...
        if img.all() != None:
            ...
            raise ValueError('The image value is None. Image') 
