...
class handDetector():
    ...
    def findMousePosition(self, img, handNo=0, draw=True):
        xList           = []
        yList           = []
        bbox            = []
        self.lmList     = []
    ...
    def fingersUp(self):
        ...
        # Thumb
        if self.lmList[self.ids[0]][1] > self.lmList[self.ids[0] - 1][1]:
            fingers.append(1)
        else:
            fingers.append(0)
        ...
        for id in range(1, 5):
            ...
            if self.lmList[self.ids[id]][2] < self.lmList[self.ids[id] - 2][2]:
                fingers.append(1)
            else:
                fingers.append(0)
